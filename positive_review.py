...
y_pred_normal = np.maximum(0, X_design_test @ beta)
from sklearn.metrics import mean_squared_error, r2_score, mean_absolute_error
import matplotlib.pyplot as plt
...
# 1. Train a new Random Forest on the ORIGINAL scaled features (X_scaled) 
# and our log-transformed target (y_log) from earlier.
rf_interpreter = RandomForestRegressor(random_state=42)
rf_interpreter.fit(X_scaled, y_log)

# 2. Extract the feature importances
importances = rf_interpreter.feature_importances_

# 3. Match the importances back to the original column names from 'X'
...
# 4. Sort the DataFrame so the most important features are at the top
importance_df = importance_df.sort_values(by="Importance", ascending=False)

# 5. Print the top 10 to the console
print("\nTop 10 Most Important Features:")
print(importance_df.head(10))

# 6. Plot the top 15 features for a nice visual
top_n = 15
plt.figure(figsize=(10, 6))
# We reverse the order ([::-1]) so the highest is at the top of the horizontal bar chart
